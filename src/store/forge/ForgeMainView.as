package store.forge
{
   ...
   public class ForgeMainView extends Sprite implements Disposeable
   {
      ...
      private var _rightBgView:store.forge.ForgeRightBgView;
      ...
      private function initView():void
      {
         ...
         this.bg.y = 31;
         addChild(this.bg);
         this._rightBgView = new store.forge.ForgeRightBgView();
         ...
         while (_loc2_ < _loc3_)
         {
            _loc1_ = ComponentFactory.Instance.creatComponentByStylename("forgeMainView.tabSelectedButton" + _loc2_);
            _loc1_.addEventListener(MouseEvent.CLICK, this.clickHandler, false, 0, true);
            this._tabVbox.addChild(_loc1_);
            this._tabSBG.addSelectItem(_loc1_);
            this._tabSBList.push(_loc1_);
            _loc2_++;
         }
         this.bg.setFrame(1);
         this._tabSBG.selectIndex = this._initIndex;
