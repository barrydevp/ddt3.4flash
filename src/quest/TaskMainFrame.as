package quest
{
   ...
   public class TaskMainFrame extends Frame
   {
      ...
      private var infoView:quest.QuestInfoPanelView;
      
      private var rewardView:quest.QuestRewardView;
      ...
      private var _currentCateView:quest.QuestCateView;
      
      public var currentNewCateView:quest.QuestCateView;
      ...
      private function initView() : void
      {
         ...
         this.addQuestList();
         this.infoView = new quest.QuestInfoPanelView();
         PositionUtils.setPos(this.infoView,"quest.infoPanelPos");
         addToContent(this.infoView);
         this.rewardView = new quest.QuestRewardView();
      ...
      {
         this._leftBGStyleNormal.visible = this._rightBGStyleNormal.visible = param1 == NORMAL ? Boolean(true) : Boolean(false);
         this._leaf.visible = this._leftBGStyleGuide.visible = this._rightBGStyleGuide.visible = param1 == GUIDE ? Boolean(true) : Boolean(false);
      }
      
      private function addQuestList() : void
      {
         var _loc1_:int = 0;
         var _loc2_:quest.QuestCateView = null;
         ...
         while(_loc1_ < 7)
         {
            _loc2_ = new quest.QuestCateView(_loc1_);
            _loc2_.collapse();
      ...
      private function __onEnbleChange(param1:Event) : void
      {
         ...
         while(_loc4_ < 6)
         {
            _loc2_ = this.cateViewArr[_loc4_];
            if(_loc2_.visible)
            {
               _loc2_.y = this.CATEVIEW_Y + this.CATEVIEW_H * _loc3_;
               _loc3_++;
            }
            _loc4_++;
         }
      ...
      private function __onCateViewChange(param1:Event) : void
      {
         ...
         {
            _loc2_ = this.cateViewArr[_loc4_] as QuestCateView;
            if(_loc2_.visible)
            {
               _loc2_.y = _loc3_;
               _loc3_ += _loc2_.contentHeight;
            }
            _loc4_++;
         }
      }
      ...
      private function __onTitleClicked(param1:Event) : void
      {
         ...
         var _loc3_:int = this.CATEVIEW_Y;
         var _loc4_:int = 0;
         while(_loc4_ < this.cateViewArr.length)
         {
            _loc2_ = this.cateViewArr[_loc4_] as QuestCateView;
            if(_loc2_ != this._currentCateView)
            {
               _loc2_.collapse();
            }
            if(_loc2_.visible)
...
//		  trace("open: " + this._opened);
      ...
      private function __onQuestBtnClicked(param1:MouseEvent) : void
      {
         ...
         SoundManager.instance.play("008");
         var _loc5_:QuestInfo = this.infoView.info;
         if(Boolean(_loc5_) && !_loc5_.isCompleted)
         {
         ...
         {
            _loc2_ = [];
            for each(_loc3_ in _loc5_.itemRewards)
            {
      ...
      {
         var _loc2_:quest.QuestCateView = null;
         var _loc3_:quest.QuestCateView = null;
         super.__onAddToStage(param1);
         var _loc4_:int = -1;
         var _loc5_:int = 0;
         while(_loc5_ < this.cateViewArr.length)
         {
            _loc2_ = this.cateViewArr[_loc5_] as QuestCateView;
            _loc2_.initData();
            _loc5_++;
         }
         var _loc6_:int = 0;
         while(_loc6_ < this.cateViewArr.length)
         {
            _loc3_ = this.cateViewArr[_loc6_] as QuestCateView;
            _loc3_.initData();
            if(_loc3_.data.haveCompleted)
            {
               _loc3_.active();
               return;
            }
            if(_loc3_.length > 0 && _loc4_ < 0)
            {
               _loc4_ = _loc6_;
               _loc3_.active();
            }
      ...
      {
         var _loc1_:quest.QuestCateView = null;
         this.hideGuide();
         this.removeEvent();
         this._currentCateView = null;
         this.currentNewCateView = null;
         while(Boolean(_loc1_ = this.cateViewArr.pop()))
         {
         ...
         if(Boolean(this._leftBGStyleGuide))
         {
         ...
         if(Boolean(this._rightBGStyleGuide))
         {
