package quest
{
   ...
   public class NewQuestView extends Frame
   {
      ...
      private var mainFrame:quest.TaskMainFrame;
      ...
      private function initData() : void
      {
         ...
         var _loc3_:TaskPannelStripView = null;
         this._questList = new Array();
         var _loc4_:Array = TaskManager.newQuests;
         for each(_loc1_ in _loc4_)
         {
            if(_loc1_.Type != 2)
            {
               this._questList.push(_loc1_);
            }
         }
         for each(_loc2_ in this._questList)
         {
            _loc3_ = new TaskPannelStripView(_loc2_);
            _loc3_.addEventListener(MouseEvent.CLICK,this.__questStripClicked);
      ...
      {
         if(Boolean(this.mainFrame.parent))
         {
            return;
         }
         this.clearList();
