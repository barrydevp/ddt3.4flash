package worldboss.view
{
   ...
   public class WorldBossScneneMap extends Sprite implements Disposeable
   {
      ...
      private const START_GAME_INTERVAL:Number = 60000;
      ...
      private var _lastStart:Number = 0;
      ...
      private function checkDistance():Boolean
      {
         ...
         var _loc2_:Number = this.selfPlayer.x - this.armyPos.x;
         var _loc3_:Number = this.selfPlayer.y - this.armyPos.y;
         if (Math.pow(_loc2_, 2) + Math.pow(_loc3_, 2) > Math.pow(this.r, 2))
         {
            _loc1_ = Math.atan2(_loc3_, _loc2_);
            this.auto = new Point(this.armyPos.x, this.armyPos.y);
            this.auto.x += (_loc2_ > 0 ? 1 : -1) * Math.abs(Math.cos(_loc1_) * this.r);
            this.auto.y += (_loc3_ > 0 ? 1 : -1) * Math.abs(Math.sin(_loc1_) * this.r);
            return false;
         }
         return true;
      }
      ...
      private function checkCanStartGame():Boolean
      {
         ...
         if (_loc1_ && this._lastStart != 0 && getTimer() - this._lastStart < this.START_GAME_INTERVAL)
         {
            MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("worldboss.startGame.msg1"));
            _loc1_ = false;
         }
      ...
      public function resetLastStart():void
      {
         this._lastStart = 0;
      }
      ...
      {
         this._lastStart = getTimer();
         SocketManager.Instance.out.createUserGuide(RoomInfo.WORLD_BOSS_FIGHT);
      ...
      private function _hideOtherPlayer(param1:WorldBossRoomEvent):void
      {
         ...
         this._isShowOther = param1.data as Boolean;
         if (Boolean(this._characters))
         {
            for each (_loc2_ in this._characters)
            {
               ...
               {
                  _loc2_.visible = this._isShowOther;
               }
            }
         }
      }
      ...
      private function addPlayerCallBack(param1:WorldRoomPlayer, param2:Boolean, param3:int):void
      {
         if (param3 == 0)
         {
            ...
            {
               param1.visible = this._isShowOther;
            }
         }
      }
      ...
      protected function BuildEntityDepth():void
      {
         ...
         var _loc8_:int = this.articleLayer.numChildren;
         var _loc9_:int = 0;
         while (_loc9_ < _loc8_ - 1)
         {
            _loc1_ = this.articleLayer.getChildAt(_loc9_);
            _loc2_ = this.getPointDepth(_loc1_.x, _loc1_.y);
            _loc4_ = Number.MAX_VALUE;
            _loc5_ = _loc9_ + 1;
            while (_loc5_ < _loc8_)
            {
               _loc6_ = this.articleLayer.getChildAt(_loc5_);
            ...
            if (_loc2_ > _loc4_)
            {
               this.articleLayer.swapChildrenAt(_loc9_, _loc3_);
            }
            _loc9_++;
         }
      }
      ...
      public function dispose():void
      {
         ...
         for each (_loc1_ in this._characters)
         {
            ...
            _loc1_.removeEventListener(SceneCharacterEvent.CHARACTER_MOVEMENT, this.setCenter);
            _loc1_.removeEventListener(SceneCharacterEvent.CHARACTER_ACTION_CHANGE, this.playerActionChange);
            _loc1_.dispose();
            _loc1_ = null;
         }
         ...
         {
            _loc2_ = this.articleLayer.numChildren;
            while (_loc2_ > 0)
            {
               _loc3_ = this.articleLayer.getChildAt(_loc2_ - 1) as WorldRoomPlayer;
               if (Boolean(_loc3_))
               {
                  _loc3_.removeEventListener(SceneCharacterEvent.CHARACTER_MOVEMENT, this.setCenter);
                  _loc3_.removeEventListener(SceneCharacterEvent.CHARACTER_ACTION_CHANGE, this.playerActionChange);
               ...
               try
               {
                  this.articleLayer.removeChildAt(_loc2_ - 1);
               }
               catch (e:RangeError)
               {
               }
