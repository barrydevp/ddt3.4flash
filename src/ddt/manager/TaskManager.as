package ddt.manager
{
   ...
   public class TaskManager
   {
      ...
      private static var _instance:ddt.manager.TaskManager;
      ...
      public static function get instance() : ddt.manager.TaskManager
      {
         if(_instance == null)
         {
            _instance = new ddt.manager.TaskManager();
         }
         return _instance;
      }
      ...
      private static function IsQuestFinish(param1:int) : Boolean
      {
         if(!_questLog)
         {
            return false;
      ...
      public static function getAvailableQuests(param1:int = -1, param2:Boolean = true) : QuestCategory
      {
         ...
         var _loc4_:QuestCategory = new QuestCategory();
         for each(_loc3_ in allQuests)
         {
            if(param1 > -1)
            {
               if(param1 == 5)
            ...
            if(param1 == 3)
            {
               if(_loc3_.id >= 2000 && _loc3_.id <= 2018)
               {
                  continue;
               }
            }
            if(param2 && _loc3_.data && !_loc3_.data.isExist)
            {
               requestQuest(_loc3_);
            }
            else if(isAvailableQuest(_loc3_,true))
            {
               if(_loc3_.Id != achievementQuestNo)
               {
                  if(_loc3_.isCompleted)
                  {
                     _loc4_.addCompleted(_loc3_);
                  }
                  else if(Boolean(_loc3_.data) && _loc3_.data.isNew)
                  {
                     _loc4_.addNew(_loc3_);
                  }
                  else
                  {
                     _loc4_.addQuest(_loc3_);
                  }
               }
            }
         }
      ...
      public static function get newQuests() : Array
      {
         ...
         {
            if(_loc1_.data && _loc1_.data.needInformed && _loc1_.Type != 2)
            {
               _loc2_.push(_loc1_);
            }
         }
      ...
      public static function get welcomeQuests() : Array
      {
         var _loc1_:QuestInfo = null;
         var _loc2_:Array = new Array();
         for each(_loc1_ in dailyQuests)
         {
            if(_loc1_.otherCondition != 1)
            {
               _loc2_.push(_loc1_);
            }
         }
      ...
      {
         if(Boolean(getQuestByID(param1)))
         {
            return getQuestByID(param1).data;
         }
         return null;
      ...
      private static function isAvailableQuest(param1:QuestInfo, param2:Boolean = false) : Boolean
      {
         ...
         var _loc7_:Array = PathManager.DISABLE_TASK_ID;
         var _loc8_:int = 0;
         while(_loc8_ < _loc7_.length)
         {
            if(param1.id == parseInt(_loc7_[_loc8_]))
            {
               return false;
            }
            _loc8_++;
         ...
         if(param1.Type <= 100)
         {
            ...
            if(param1.NeedMinLevel > _loc3_ || param1.NeedMaxLevel < _loc3_)
            {
               return false;
            }
         }
         ...
         {
            _loc4_ = [];
            _loc4_ = param1.PreQuestID.split(",");
            for each(_loc5_ in _loc4_)
            {
               if(_loc5_ != 0)
               {
                  if(Boolean(getQuestByID(_loc5_)))
                  {
      ...
      private static function __updateAcceptedTask(param1:CrazyTankSocketEvent) : void
      {
         ...
         while(_loc11_ < _loc10_)
         {
            ...
            _loc3_ = new QuestInfo();
            if(Boolean(getQuestByID(_loc2_)))
            {
               _loc3_ = getQuestByID(_loc2_);
               if(Boolean(_loc3_.data))
               {
                  _loc4_ = _loc3_.data;
               }
               else
               {
                  _loc4_ = new QuestDataInfo(_loc2_);
                  if(_loc3_.required)
                  {
                     _loc4_.isNew = true;
                  }
               ...
               _loc4_.isAchieved = _loc9_.readBoolean();
               _loc5_ = _loc9_.readInt();
               _loc6_ = _loc9_.readInt();
               _loc7_ = _loc9_.readInt();
               _loc8_ = _loc9_.readInt();
			   // Conditions
               _loc4_.setProgress(_loc5_,_loc6_,_loc7_,_loc8_);
               _loc4_.CompleteDate = _loc9_.readDate();
			   // repeatLeft = RepeatFinish
               _loc4_.repeatLeft = _loc9_.readInt();
			   // quality = Randobule
               _loc4_.quality = _loc9_.readInt();
               _loc4_.isExist = _loc9_.readBoolean();
               _loc3_.data = _loc4_;
               if(_loc4_.isNew)
               {
                  addNewQuest(_loc3_);
               }
               if(PetBagController.instance().isPetFarmGuildeTask(_loc3_.QuestID))
               {
                  PetBagController.instance().dispatchEvent(new UpdatePetFarmGuildeEvent(UpdatePetFarmGuildeEvent.FINISH,_loc3_));
               }
               _dispatcher.dispatchEvent(new TaskEvent(TaskEvent.CHANGED,_loc3_,_loc4_));
            }
            _loc11_++;
         }
         loadQuestLog(_loc9_.readByteArray());
      ...
      public static function requestCanAcceptTask() : void
      {
         ...
         var _loc3_:Array = allAvailableQuests;
         if(_loc3_.length != 0)
         {
            _loc1_ = new Array();
      ...
      public static function hasConsortiaSayTask() : Boolean
      {
         ...
         for each(var _loc6_ in _loc3_)
         {
            ...
            {
               for each(var _loc8_ in _loc1_._conditions)
               {
                  _loc2_ = _loc8_;
                  _loc8_;
                  if(_loc2_.type == 20 && _loc2_.param == 4)
                  {
                     return true;
                  }
               }
      ...
      public static function sendQuestData(param1:int, param2:int) : void
      {
         ...
         var _loc3_:int = int(getQuestByID(param1).data.progress[param2]);
         _loc3_--;
      ...
      public static function checkHighLight() : void
      {
         ...
         for each(var _loc5_ in allCurrentQuest)
         {
            _loc1_ = _loc5_;
            _loc5_;
            if(!_loc1_.isAchieved || _loc1_.CanRepeat)
            {
               if(_loc1_.isCompleted)
               {
                  if(!_loc1_.hadChecked)
                  {
                     _loc2_++;
                  }
               }
