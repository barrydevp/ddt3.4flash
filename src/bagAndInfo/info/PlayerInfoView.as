package bagAndInfo.info
{
   ...
   public class PlayerInfoView extends Sprite implements Disposeable
   {
      ...
      private var _playerInfoEffortHonorView:bagAndInfo.info.PlayerInfoEffortHonorView;
      ...
      private var _attackButton:bagAndInfo.info.GlowPropButton;
      
      private var _agilityButton:bagAndInfo.info.GlowPropButton;
      
      private var _defenceButton:bagAndInfo.info.GlowPropButton;
      
      private var _luckButton:bagAndInfo.info.GlowPropButton;
      
      private var _damageTxt:FilterFrameText;
      
      private var _damageButton:bagAndInfo.info.PropButton;
      
      private var _armorTxt:FilterFrameText;
      
      private var _armorButton:bagAndInfo.info.PropButton;
      
      private var _HPText:FilterFrameText;
      
      private var _hpButton:bagAndInfo.info.PropButton;
      
      private var _vitality:FilterFrameText;
      
      private var _vitalityBuntton:bagAndInfo.info.PropButton;
      
      private var _textLevelPrpgress:FilterFrameText;
      
      private var _progressLevel:bagAndInfo.info.LevelProgress;
      ...
      private var _dragDropArea:bagAndInfo.info.PersonalInfoDragInArea;
      ...
      private function initView() : void
      {
         ...
         addChild(this._bg);
         this._dragDropArea = new bagAndInfo.info.PersonalInfoDragInArea();
         ...
         this._playerInfoEffortHonorView = new bagAndInfo.info.PlayerInfoEffortHonorView();
         if(PathManager.solveAchieveEnable())
         {
      ...
      {
         SoundManager.instance.playButtonSound();
         this._ringSystemView = ComponentFactory.Instance.creatComponentByStylename("bagAndInfo.bag.ringSystemView");
         LayerManager.Instance.addToLayer(this._ringSystemView,3,true,1);
         this._ringSystemView.addEventListener("response",this.__onRingSystemClose);
      }
      ...
      private function __lightPlay(param1:TaskEvent) : void
      {
         ...
         if(Boolean(TaskManager.achievementQuest.data) && TaskManager.achievementQuest.data.progress[0] <= 0)
         {
            this._achvLight.play();
         }
         else
      ...
      {
         SoundManager.instance.play("008");
         if(Boolean(this._info) && PlayerManager.Instance.Self.ID != this._info.ID)
         {
      ...
      private function updateCells() : void
      {
         ...
         {
            _loc1_.info = this._info == null ? null : this._info.Bag.getItemAt(_loc1_.place);
            if(Boolean(_loc1_.info))
            {
               (_loc1_.info as InventoryItemInfo).gemstoneList = GemstoneManager.Instance.getByPlayerInfoList(_loc1_.place,this._info.ID);
            }
         }
      ...
      private function updateIcons() : void
      {
         ...
         if(Boolean(this._info))
         {
            ...
            else if(Boolean(this._academyIcon))
            {
            ...
            if(Boolean(this._explorerIcon) && PlayerManager.Instance.Self.Grade >= 25)
            {
               this._explorerIcon.visible = true;
               this._explorerIcon.tipData = this._info.manualProInfo;
               _loc2_ = this._info.manualProInfo.manual_Level;
               this._explorerIcon.setFrame(Math.min(int((_loc2_ - 1) / 5) + 1,4));
            }
            else
            {
               this._explorerIcon.visible = false;
            }
            this._explorerIcon.buttonMode = this._info.isSelf && this._showSelfOperation;
         }
         else
         {
            if(Boolean(this._levelIcon))
      ...
      private function updatePersonInfo() : void
      {
         ...
         if(Boolean(this._info))
         {
            ...
            _loc2_ = _loc2_ > _loc1_ ? int(_loc1_) : int(_loc2_);
      ...
      public function dispose() : void
      {
         ...
         this._marriedIcon = null;
         if(Boolean(this._hiddenControlsBg))
         {
      ...
      private function updateShowOperation() : void
      {
         ...
         this._achvBtn.visible = Boolean(this._info) && (this._info.ZoneID == 0 || this._info.ZoneID == PlayerManager.Instance.Self.ZoneID) && this._achvEnable;
         ...
         this._ringSystemBtn.visible = this._showSelfOperation && this._showEquip.visible;
         if(this._showSelfOperation && this._showEquip.visible)
         {
            this._openNecklacePtetrochemicalView.visible = true;
            this._ringSystemBtn.visible = true;
