package bagAndInfo.bag
{
   ...
   public class BagView extends Sprite
   {
      ...
      protected var _equiplist:bagAndInfo.bag.BagEquipListView;
      
      protected var _proplist:bagAndInfo.bag.BagListView;
      
      protected var _petlist:bagAndInfo.bag.PetBagListView;
      
      protected var _sellBtn:bagAndInfo.bag.SellGoodsBtn;
      
      protected var _continueBtn:bagAndInfo.bag.ContinueGoodsBtn;
      
      protected var _lists:Array;
      
      protected var _currentList:bagAndInfo.bag.BagListView;
      
      protected var _breakBtn:bagAndInfo.bag.BreakGoodsBtn;
      
      protected var _keySetBtn:SimpleBitmapButton;
      
      private var _keySetFrame:bagAndInfo.bag.KeySetFrame;
      ...
      protected var _goldButton:bagAndInfo.bag.RichesButton;
      
      protected var _giftButton:bagAndInfo.bag.RichesButton;
      
      protected var _moneyButton:bagAndInfo.bag.RichesButton;
      
      protected var _medalButton:bagAndInfo.bag.RichesButton;
      ...
      private const _TRIEVENEEDLEVEL:int = 60;
      ...
      {
         this._equiplist = new bagAndInfo.bag.BagEquipListView(0,31,79,7,this._equipBagPage);
         this._proplist = new BagProListView(1,0,48);
         this._petlist = new bagAndInfo.bag.PetBagListView(1);
      ...
      private function __upData(param1:DictionaryEvent) : void
      {
         ...
         var _loc5_:int = _loc4_.Place % 4 == 0 ? int(int(_loc4_.Place / 4 - 2)) : int(int(_loc4_.Place / 4 - 1));
         var _loc6_:int = _loc4_.Place % 4 == 0 ? int(int(4)) : int(int(_loc4_.Place % 4));
      ...
      private function __sellClick(param1:MouseEvent) : void
      {
         if(!(this.state & this.STATE_SELL))
         {
            ...
            this._sellBtn.addEventListener(bagAndInfo.bag.SellGoodsBtn.StopSell,this.__stopSell);
      ...
      {
         this.state = ~this.STATE_SELL & this.state;
         this._sellBtn.removeEventListener(bagAndInfo.bag.SellGoodsBtn.StopSell,this.__stopSell);
      ...
      protected function __cellDoubleClick(param1:CellEvent) : void
      {
         ...
         var _loc5_:InventoryItemInfo = _loc4_.info as InventoryItemInfo;
         var _loc6_:ItemTemplateInfo = ItemManager.Instance.getTemplateById(_loc5_.TemplateID);
         var _loc7_:int = PlayerManager.Instance.Self.Sex ? int(int(1)) : int(int(2));
         if(_loc5_.getRemainDate() <= 0)
         {
            return;
         }
         if(_loc6_.NeedSex != _loc7_ && _loc6_.NeedSex != 0)
         {
         ...
         if(!_loc4_.locked)
         {
            if((_loc4_.info.BindType == 1 || _loc4_.info.BindType == 2 || _loc4_.info.BindType == 3) && _loc4_.itemInfo.IsBinds == false)
            {
            ...
            {
               SoundManager.instance.play("008");
               if(PlayerManager.Instance.Self.canEquip(_loc5_))
               {
                  if(_loc5_.CategoryID == 50 || _loc5_.CategoryID == 51 || _loc5_.CategoryID == 52)
                  {
                     if(PetBagController.instance().view && PetBagController.instance().view.parent)
                     {
                        ...
                        SocketManager.Instance.out.addPetEquip(_loc4_.place,PetBagController.instance().petModel.currentPetInfo.Place,BagInfo.EQUIPBAG);
                     }
                     return;
                  }
                  _loc3_ = PlayerManager.Instance.getDressEquipPlace(_loc5_);
                  SocketManager.Instance.out.sendMoveGoods(BagInfo.EQUIPBAG,_loc5_.Place,BagInfo.EQUIPBAG,_loc3_,_loc5_.Count);
               }
            }
         }
      }
      ...
      protected function __cellOpen(param1:Event) : void
      {
         ...
         var _loc7_:BagCell = CellMenu.instance.cell as BagCell;
         this._currentCell = _loc7_;
         if(_loc7_ != null && _loc7_.itemInfo != null)
         {
            if(PlayerManager.Instance.Self.bagLocked)
            {
               BaglockedManager.Instance.show();
            ...
            _loc2_ = PlayerManager.Instance.Self.Sex ? Number(Number(1)) : Number(Number(2));
            if(_loc7_.info.NeedSex != 0 && _loc2_ != _loc7_.info.NeedSex)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("tank.view.bagII.sexErr"));
               return;
            }
            if(PlayerManager.Instance.Self.Grade >= _loc7_.info.NeedLevel)
            {
               if(_loc7_.info.TemplateID == EquipType.VIP_COIN && !PlayerManager.Instance.Self.IsVIP)
               {
                  param1.stopImmediatePropagation();
                  MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.vip.vipIcon.notVip"));
               }
               else if(_loc7_.info.TemplateID == EquipType.ROULETTE_BOX)
               {
                  RouletteManager.instance.useRouletteBox(_loc7_);
               }
               else if(_loc7_.info.TemplateID == EquipType.SURPRISE_ROULETTE_BOX)
               {
                  param1.stopImmediatePropagation();
                  RouletteManager.instance.useSurpriseRoulette(_loc7_);
               }
               else if(EquipType.isCaddy(_loc7_.info))
               {
                  param1.stopImmediatePropagation();
                  RouletteManager.instance.useCaddy(_loc7_);
               }
               else if(EquipType.isBeadNeedOpen(_loc7_.info))
               {
                  param1.stopImmediatePropagation();
                  RouletteManager.instance.useBead(_loc7_.info.TemplateID);
               }
               else if(EquipType.isOfferPackage(_loc7_.info))
               {
                  param1.stopImmediatePropagation();
                  RouletteManager.instance.useOfferPack(_loc7_);
               }
               else if(EquipType.isTimeBox(_loc7_.info))
               {
                  _loc3_ = DateUtils.getDateByStr(InventoryItemInfo(_loc7_.info).BeginDate);
                  _loc4_ = int(_loc7_.info.Property3) * 60 - (TimeManager.Instance.Now().getTime() - _loc3_.getTime()) / 1000;
                  if(_loc4_ <= 0)
                  {
                     SocketManager.Instance.out.sendItemOpenUp(_loc7_.itemInfo.BagType,_loc7_["place"]);
                  }
                  else
                  {
                     _loc5_ = _loc4_ / 3600;
                     _loc6_ = _loc4_ % 3600 / 60;
                     _loc6_ = _loc6_ > 0 ? int(int(_loc6_)) : int(int(1));
                     MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.userGuild.boxTip",_loc5_,_loc6_));
                  }
               }
               else if(_loc7_.info.CategoryID == EquipType.CARDBOX)
               {
                  param1.stopImmediatePropagation();
                  RouletteManager.instance.useCard(_loc7_);
               }
               else if(_loc7_.info.TemplateID == EquipType.MY_CARDBOX)
               {
                  RouletteManager.instance.useBead(_loc7_.info.TemplateID);
               }
               else if(EquipType.PET_EGG == _loc7_.info.CategoryID)
               {
                  SocketManager.Instance.out.sendAddPet(_loc7_.itemInfo.Place,_loc7_.itemInfo.BagType);
               }
               else if(_loc7_.info.TemplateID == 1120434 || _loc7_.info.TemplateID == 12545)
               {
                  SocketManager.Instance.out.sendChangeSex(_loc7_.bagType,_loc7_.place);
               }
               else if(_loc7_.info.CategoryID == 72)
               {
                  SocketManager.Instance.out.sendItemOpenUp(_loc7_.itemInfo.BagType,_loc7_.itemInfo.Place);
               }
               else
               {
                  SocketManager.Instance.out.sendItemOpenUp(_loc7_.itemInfo.BagType,_loc7_["place"]);
      ...
      private function __cellUse(param1:Event) : void
      {
         ...
         if(_loc3_.info.TemplateID == EquipType.REWORK_NAME)
         {
            this.startReworkName(_loc3_.bagType,_loc3_.place);
            return;
         }
         if(_loc3_.info.CategoryID == 11 && _loc3_.info.Property1 == "5" && _loc3_.info.Property2 != "0")
         {
            this.showChatBugleInputFrame(_loc3_.info.TemplateID);
            return;
         }
         if(_loc3_.info.CategoryID == EquipType.TEXP_TASK)
         {
            ...
            if(TexpManager.Instance.getLv(TexpManager.Instance.getExp(int(_loc3_.info.Property1))) >= PlayerManager.Instance.Self.Grade + 5)
            {
            ...
            {
               this._tmpCell = _loc3_;
               _loc2_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("texpSystem.view.TexpView.refreshTaskTip"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),true,true,true,LayerManager.ALPHA_BLOCKGOUND);
               _loc2_.addEventListener(FrameEvent.RESPONSE,this.__texpResponse);
               return;
            }
            SocketManager.Instance.out.sendTexp(-1,_loc3_.info.TemplateID,_loc3_.place);
            return;
         }
         if(_loc3_.info.TemplateID == EquipType.CONSORTIA_REWORK_NAME)
         {
            ...
            this.startupConsortiaReworkName(_loc3_.bagType,_loc3_.place);
            return;
         }
         if(_loc3_.info.TemplateID == EquipType.CHANGE_SEX)
         {
         ...
         if(_loc3_.info.CategoryID == EquipType.TITLE_CARD)
         {
            SocketManager.Instance.out.sendNewTitleCard(_loc3_.itemInfo.Place,_loc3_.itemInfo.BagType);
         }
         if(_loc3_.info.CategoryID == 11 && int(_loc3_.info.Property1) == 37)
         {
            ...
            {
               SocketManager.Instance.out.sendUseChangeColorShell(_loc3_.bagType,_loc3_.place);
               return;
            }
            MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("bagAndInfo.bag.UnableUseColorShell"));
         }
         if(_loc3_.info.TemplateID == EquipType.COLORCARD)
         {
            if(!this._changeColorController)
            {
               this._changeColorController = new ChangeColorController();
            }
            this._changeColorController.changeColorModel.place = _loc3_.place;
            UIModuleSmallLoading.Instance.progress = 0;
            UIModuleSmallLoading.Instance.show();
         ...
         else if(_loc3_.info.TemplateID != EquipType.TRANSFER_PROP)
         {
            ...
            else
            {
               ...
               else if(_loc3_.info.TemplateID == 12536 || _loc3_.info.TemplateID == 12537)
               {
                  if(PlayerManager.Instance.Self.IsMarried)
                  {
                     SocketManager.Instance.out.sendUseLoveFeelingly(_loc3_.itemInfo.BagType,_loc3_["place"]);
                  }
                  else
                  {
               ...
               else if((_loc3_.info.CategoryID == 11 || _loc3_.info.CategoryID == 72) && int(_loc3_.info.Property1) == 6)
               {
                  this.__cellOpen(param1);
               }
               else
               {
                  this.useCard(_loc3_.itemInfo);
               }
            }
         }
      }
      ...
      private function judgeAndPlayCardMovie() : void
      {
         ...
         var _loc6_:ItemTemplateInfo = this._currentCell.info;
         var _loc7_:DictionaryData = PlayerManager.Instance.Self.cardBagDic;
         for each(_loc1_ in _loc7_)
         {
            if(_loc1_.TemplateID == int(_loc6_.Property5))
            {
               return;
            }
         }
         ...
         _loc3_ = ItemManager.Instance.getTemplateById(int(_loc6_.Property5));
         _loc4_ = new BaseCell(_loc2_,_loc3_);
         this.getNewCardMovie["card"].addChild(_loc4_);
         _loc5_ = ComponentFactory.Instance.creatComponentByStylename("getNewCardMovie.text");
         _loc5_.text = LanguageMgr.GetTranslation("ddt.cardSystem.getNewCard.name",_loc3_.Name);
         _loc5_.x -= (_loc5_.textWidth - _loc4_.width) / 6;
         this.getNewCardMovie["word"].addChild(_loc5_);
      ...
      private function __changeSexHandler(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(_loc3_)
         {
            ...
            _loc2_.info = this.getAlertInfo("tank.view.bagII.changeSexAlert.success",false);
            _loc2_.addEventListener(ComponentEvent.PROPERTIES_CHANGED,this.__onAlertSizeChanged);
            _loc2_.addEventListener(FrameEvent.RESPONSE,this.__onSuccessAlertResponse);
            LayerManager.Instance.addToLayer(_loc2_,LayerManager.GAME_DYNAMIC_LAYER,_loc2_.info.frameCenter,LayerManager.BLCAK_BLOCKGOUND);
